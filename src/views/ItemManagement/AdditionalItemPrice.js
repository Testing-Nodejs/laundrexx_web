...
const AdditionalItemPrice = () => {
    ...
    const UpdateItemPrice = () => {
        axios
            .put(MyApiUrl + "AdditionalServicePrice/" + ServiceCategory + "", { Items: ItemsData })
            .then((response) => {
                if (response.data === true) {
                    Toast.fire({
                        icon: "success",
                        title: "Additional Services Price Updated!",
                    });
                } else {
                    Toast.fire({
                        icon: "error",
    ...
    const ExportToExcel = () => {
        ...
        csvData.push(
            '"Sl No","Additional Service Name","Tier 1","Tier 2","Tier 3","Tier 4"'
        );
        if (DisplayType === "0") {
            ItemFileName = "Additional_Service_InActive_List.csv";
            ItemsData.map((item) => {
                if (item.PriceStatus === "-") {
                    return (
                        cnt++,
            ...
            ItemFileName = "Additional_Service_Active_List.csv";
            ItemsData.map((item) => {
                if (item.PriceStatus !== "-") {
                    return (
                        cnt++,
            ...
            ItemFileName = "Additional_Service_All.csv";
            ItemsData.map((item) => {
                return (
                    cnt++,
                    csvData.push(
        ...
        <div className="123">
            ...
            <CRow style={{ marginTop: "1%", borderBottom: "1px solid #d4d4d4", paddingBottom: "2%" }}>
                    ...
                    <CSelect
                        ...
                        onChange={(event) => {
                            if (event.target.value === "-1") {
                                ...
                                axios.get(MyApiUrl + "AdditionalServiceForPrice/" + event.target.value + "").then((response) => {
                                    if (response.data.length !== 0) {
                                        setItemsData(response.data);
                                        setItemsData2(response.data);
                                    }
                                    else {
                                        setItemsData([]);
                                        setItemsData2([]);
                ...
                <CCol md="7" style={{ alignSelf: "center" }}>
                    <h1 id="ccmaster">Additional Service Price Management</h1>
                </CCol>
                <CCol md="2" style={{ alignSelf: "center" }}>
                    <CSelect
                        custom
                        name="RouteCode"
                        id="RouteCode"
                                                    ...
                                                    if (item.PriceStatus === "-") {
                                                        return (
                                                            <React.Fragment>
                                                                <tr>
                                                                    <td>{i + 1}</td>
                                                                    <td>{item.ADDITIONAL_SERVICE_NAME}</td>
                                                                    <td>
                                                                        <CInput
                                                                            type="text"
                                                                            name={item.ADDITIONAL_SERVICE_PKID}
                                                                            value={item.tier1 === 0 ? "" : item.tier1}
                                                                            placeholder="Enter Price"
                                                                            onChange={(event) => {
                                                                                const updatedData = ItemsData.map((item) => {
                                                                                    if (item.ADDITIONAL_SERVICE_PKID === parseInt(event.target.name)) {
...
export default AdditionalItemPrice;

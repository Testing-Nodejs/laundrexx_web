...
const Items = () => {
  ...
  const addMerchant = () => {
    ...
    } else if (subcategory == "-1" || subcategory == null) {
      Toast.fire({
        icon: "warning",
        title: "Please Select Item Sub Category!",
      });
    ...
    } else if (ItemDisplayName == "" || ItemDisplayName == null) {
      Toast.fire({
        icon: "warning",
        title: "Please Enter Item Display Name!",
      });
    } else {
      const obj = {
        ITEMS_CATEGORY_FKID: category,
        ITEMS_SUB_CATEGORY_FKID: subcategory,
        ITEMS_NAME: ItemName,
        ITEMS_DISPLAY_NAME: ItemDisplayName
      };
      axios
        .post(MyApiUrl + "Items", obj)
        .then((response) => {
          if (response.data == "0") {
            Toast.fire({
              icon: "error",
  ...
  const UpdateCategory = () => {
    if (category == "-1" || category == null) {
      ...
      axios
        .put(MyApiUrl + "Items/" + categoryPkid + "", obj)
        .then((response) => {
          if (response.data == "0") {
            Toast.fire({
              icon: "error",
  ...
  const EditCategory = (id, category, subcategory, itemname, itemdisplayname) => {
    getSubCategoryforUpdate(category, subcategory);
    setCategoryPkid(id);
  ...
  const fetchData = React.useCallback(() => {
    axios.get(MyApiUrl + "Items").then((response) => {
  ...
  const getSubCategory = (CatID) => {
    axios.get(MyApiUrl + "ItemSubCategory/" + CatID + "").then((response) => {
      if (response.data.length != 0) {
        const CategoryOption = response.data.map((item) => (
          <option value={item.SUB_CATEGORY_PKID}>{item.SUB_CATEGORY_NAME}</option>
        ));
  ...
  const getSubCategoryforUpdate = (CatID, SubCatID) => {
    axios.get(MyApiUrl + "ItemSubCategory/" + CatID + "").then((response) => {
      if (response.data.length != 0) {
        const CategoryOption = response.data.map((item) => (
          <option value={item.SUB_CATEGORY_PKID}>{item.SUB_CATEGORY_NAME}</option>
  ...
  return (
    <div className="123">
      ...
      <h1 id="ccmaster">Manage Items</h1>
      <CRow style={{ marginTop: "3%" }}>
        <CCol md="5">
          <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
            <CCardHeader>Add/Update Items</CCardHeader>
            <CCardBody>
              <CForm action="" method="post">
                ...
                <CFormGroup row>
                  <CCol xs="12" md="12">
                    <CLabel>Item Sub Category <span style={{ color: "red" }}>*</span></CLabel>
                    <CSelect
                      custom
                      name="ProductCategory"
                      id="ProductCategory"
                    ...
                    >
                      <option value="-1">Select Item Sub Category</option>
                      {SubCategoryData}
                    </CSelect>
                  </CCol>
                </CFormGroup>
                <CFormGroup>
                  <CLabel htmlFor="">Item Name <span style={{ color: "red" }}>*</span></CLabel>
                  <CInput
                    type="text"
                    id="Items"
                    name="Items"
                ...
                <CFormGroup>
                  <CLabel htmlFor="">Item Display Name <span style={{ color: "red" }}>*</span></CLabel>
                  <CInput
                    type="text"
                    id="Items"
                    name="Items"
                    ...
                    onChange={(event) => {
                      setItemDisplayName(event.target.value);
                    }}
        ...
        <CCol md="7">
          <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
            <CCardHeader>View Items</CCardHeader>
            <CCardBody>
              <CDataTable
                items={responseData}
                fields={fields}
                    ...
                    <td>
                      <CButton
                        onClick={() => {
                          EditCategory(
                            item.ITEMS_PKID,
                            item.ITEMS_CATEGORY_FKID,
                            item.ITEMS_SUB_CATEGORY_FKID,
                            item.ITEMS_NAME,
                            item.ITEMS_DISPLAY_NAME,
                          );
                        }}
                        size="sm"
                        id="war-btn"
                      ...
                      <CButton
                        ...
                        id="war-btn1"
                        onClick={() => {
                          DeleteCategory(item.ITEMS_PKID);
                        }}
                      >
                        <DeleteSharpIcon />
                      </CButton>

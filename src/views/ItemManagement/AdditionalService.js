...
const fields = [{ key: "Action" }, { key: "Service Category" }, { key: "Additonal Service" }];
...
const AdditionalService = () => {
  ...
  const addMerchant = () => {
    ...
    } else if (AdditionalService == "" || AdditionalService == null) {
      Toast.fire({
        icon: "warning",
        title: "Please Enter Additional Service!",
      });
    } else {
      const obj = {
        ADDITIONAL_SERVICE_CATEGORY_FKID: category,
        ADDITIONAL_SERVICE_NAME: AdditionalService,
      };
      axios
        .post(MyApiUrl + "AdditionalService", obj)
        .then((response) => {
          if (response.data == "0") {
            Toast.fire({
              icon: "error",
              title: "Additional Service Already Exist!",
            });
  ...
  const UpdateCategory = () => {
    if (category == "-1" || category == null) {
      ...
      alert("Please Enter Additional Service!");
    } else {
      const obj = {
        ADDITIONAL_SERVICE_CATEGORY_FKID: category,
        ADDITIONAL_SERVICE_NAME: AdditionalService,
      ...
      axios
        .put(MyApiUrl + "AdditionalService/" + Pkid + "", obj)
        .then((response) => {
          if (response.data == "0") {
            Toast.fire({
              icon: "error",
            ...
            Toast.fire({
              icon: "success",
              title: "Additional Service Details Updated!",
            });
            reset();
          }
        });
  ...
  const EditCategory = (id, category, additionalservice) => {
    setPkid(id);
  ...
  const DeleteCategory = (id) => {
    ...
    if (res) {
      axios({
        method: "DELETE",
        url: MyApiUrl + "AdditionalService/" + id + "",
        headers: {
        ...
        .then((response) => {
          if (response.data === true) {
            Swal.fire({
              title: "Selected Additional Service Deleted!",
              icon: "success",
            ...
            Swal.fire({
              title: "Failed To Delete Additional Service!",
              icon: "error",
              confirmButtonText: "OK",
            });
            document.getElementById("divLoading").className = "hide";
  ...
  const fetchData = React.useCallback(() => {
    axios.get(MyApiUrl + "AdditionalService").then((response) => {
      if (response.data.length != 0) {
        const items = response.data.map((item) => {
          return {
            ...item,
            "Service Category": item.SERVICE_CATEGORY_NAME,
            "Additonal Service": item.ADDITIONAL_SERVICE_NAME
          };
        });
  ...
  return (
    <div className="123">
      ...
      <h1 id="ccmaster">Manage Additional Service</h1>
      <CRow style={{ marginTop: "3%" }}>
        <CCol md="5">
          <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
            <CCardHeader>Add/Update Additional Service</CCardHeader>
            <CCardBody>
              <CForm action="" method="post">
                ...
                <CFormGroup>
                  <CLabel htmlFor="">Additional Service <span style={{ color: "red" }}>*</span></CLabel>
                  <CInput
                    type="text"
                    ...
                    onChange={(event) => {
                      ...
                      value = value.replace(/\w+/g,
                        function (w) { return w[0].toUpperCase() + w.slice(1).toLowerCase(); }); setAdditionalService(value)
                    }}
        ...
        <CCol md="7">
          <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
            <CCardHeader>View Additional Service</CCardHeader>
            <CCardBody>
              <CDataTable
                items={responseData}
                fields={fields}
                  ...
                  Action: (item) => (
                    <td>
                      <CButton
                        onClick={() => {
                          EditCategory(
                            item.ADDITIONAL_SERVICE_PKID,
                            item.ADDITIONAL_SERVICE_CATEGORY_FKID,
                      ...
                      <CButton
                        ...
                        id="war-btn1"
                        onClick={() => {
                          DeleteCategory(item.ADDITIONAL_SERVICE_PKID);
                        }}
                      >
                        <DeleteSharpIcon />
                      </CButton>

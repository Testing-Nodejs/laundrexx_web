...
const OutletInvoice = (props) => {
    console.log(props);
    const InvoiceData = props.ModifiedData1;
    const InvoiceItemsData = props.InvoiceItemsData;
    ...
    const printModalInvoice = async () => {
        ...
        const OutletInvoiceWindow = window.open("", "Outlet Invoice Window", "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=auto,top=" + (screen.height - 400) + ",left=" + outletInvoiceWindowLeft);


        OutletInvoiceWindow.document.write('<html><head><title>Outlet Invoice</title></head><body style="font-family: Arial; font-size: 9px; letter-spacing: 1px">');
        OutletInvoiceWindow.document.write(document.getElementById("ModalOutletInvoiceWindow").innerHTML);
        OutletInvoiceWindow.document.write('</body></html>');
        OutletInvoiceWindow.document.close();

        // Set timeout for printing and closing each window
        setTimeout(() => {
            OutletInvoiceWindow.print();
            OutletInvoiceWindow.close();
        }, 2000);
    }
    ...
    return (
        <div>
            <div style={{ position: "absolute", display: 'none', marginTop: "10%", textAlign: "center", marginBottom: "5%", height: "100%", width: "190px" }} id="ModalOutletInvoiceWindow">
                        ...
                        <center>
                            <table width="525" className="bottom" cellspacing="0" cellpadding="2" style={{ borderLeft: "1px solid #000", borderTop: "1px solid #000", }}>
                                ...
                                <tr style={{ textAlign: "center" }}>
                                    <td colspan="6" style={{
                                        ...
                                        <h3 style={{ margin: "2px 0px 0px 0px", padding: "0px 0px 0px 0px", fontSize: 10 }}>
                                            INVOICE</h3>
                                        <span style={{ float: "right", fontSize: "10px" }}>(Factory Copy)</span>
                                ...
                                <tr style={{
                                    ...
                                    <td style={{ fontWeight: "300" }}>Time of Supply</td>
                                    <td>:</td>
                                    <td style={{ borderRight: "1px solid #000" }}> 7 P.M</td>
                                    {InvoiceData[0].ORDER_IS_PICKUP == true ?
                                        <React.Fragment>
                                            <td style={{ fontWeight: "300" }}>Pick-Up Code</td>
                                            <td>:</td>
                                            <td style={{ borderRight: "1px solid #000" }}> {InvoiceData[0].ORDER_IS_PICKUP_ID}</td>
                                        </React.Fragment>
                                        :
                            ...
                            <table width="525" border="0" cellspacing="0" cellpadding="2" style={{ borderTop: "1px solid #000", fontSize: 10 }}>
                                ...
                                {InvoiceItemsData.length > 0 ?
                                    InvoiceItemsData.map((item, index) => {
                                        footerCount = footerCount + item.ORDER_ITEM_COUNT;
                                        return (
                                            <tr>
                                                <td style={{
                                                    borderBottom: "thin solid",
                                                ...
                                                }}>{item.ORDER_ITEM_AMOUNT}</td>
                                                <td style={{
                                                    borderBottom: "thin solid",
                                                    borderRight: "thin solid", textAlign: "left",
                                                }}>{item.ADDITIONAL_SERVICE_NAME == "" || item.ADDITIONAL_SERVICE_NAME == null ? "None" : item.ADDITIONAL_SERVICE_NAME}/₹{item.ORDER_ITEM_ADDITIONAL_REQUEST_AMOUNT}</td>
                                                <td style={{
                                                    borderBottom: "thin solid",
                                                    borderRight: "thin solid", textAlign: "right",
                                                }}>{item.ORDER_ITEM_QUANTITY}</td>
                                                <td style={{
                                                    borderBottom: "thin solid",
                                                    borderRight: "thin solid", textAlign: "right",
                                                }}>{item.ORDER_ITEM_COUNT}</td>
                                                <td style={{
                                                    borderBottom: "thin solid",
                                                    borderRight: "thin solid", textAlign: "left",
                                                }}>{item.ORDER_ITEM_DEFECT}</td>
                                                <td style={{
                                                    borderBottom: "thin solid",
                                                    borderRight: "thin solid", textAlign: "right",
                                                }}>{item.ORDER_ITEM_TOTAL_AMOUNT}</td>
                                                <td style={{
                                                    borderBottom: "thin solid",
                                                    borderRight: "thin solid", textAlign: "right",
                                                }}>{item.ORDER_ITEM_DISCOUNT}</td>
                                                <td style={{
                                                    borderBottom: "thin solid",
                                                    borderRight: "thin solid", textAlign: "right",
                                                }}>{item.ORDER_ITEM_FINAL_AMOUNT}</td>
                                            </tr>
                                        )
                                    }
                                    )

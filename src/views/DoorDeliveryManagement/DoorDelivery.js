...
const DoorDelivery = () => {
    ...
    const getAllOrders = () => {
        ...
        if (SessionType == "Manager") {
            axios.get(MyApiUrl + "AllManagerDoorDeliveryOrders/" + UserID + "").then((response) => {
            ...
            axios.get(MyApiUrl + "AllAdminDoorDeliveryOrders").then((response) => {
                const items = response.data.map((item, index) => {
                    return {
                        ...item,
                        "Sl No.": index + 1,
    ...
    const filterData = (outlet, month, year) => {
        ...
        if (SessionType == "Manager") {
            ...
            document.getElementById("divLoading").className = "show";
            axios.post(MyApiUrl + "AllManagerDoorDeliveryOrdersWithFilter", object).then((response) => {
                if (response.data.length > 0) {
                    const items = response.data.map((item, index) => {
                        return {
                            ...item,
            ...
            document.getElementById("divLoading").className = "show";
            axios.post(MyApiUrl + "AllAdminDoorDeliveryOrdersWithFilter", object).then((response) => {
                if (response.data.length > 0) {
                    const items = response.data.map((item, index) => {
                        return {
                            ...item,
    ...
    const AssignDriver = (List) => {
        setOrderPkid(List.ORDER_PKID);
        setOrderID(List.ORDER_ORDER_NUMBER);
        setCustomerName(List.CUSTOMER_NAME);
        setOrderDate(List.ORDER_DATE);
    ...
    const UpdatePickupDriver = () => {
        ...
        if (Driver === "-1") {
            ...
            console.log(obj)
            axios.post(MyApiUrl + "OrderAssignDeliveryPerson", obj).then((response) => {
    ...
    const EditDriver = (List) => {
        setAssignedID(List.ASSIGNED_DOOR_DELIVERY_PKID);
        setBlock5(!block5);
        setBlock6(!block6);
        AssignDriver(List);
        setOrderPkid(List.ORDER_PKID);
    ...
    const UpdateAssignedDriver = () => {
        ...
        if (Driver === "-1") {
            ...
            console.log(obj)
            axios.put(MyApiUrl + "OrderUpdateDeliveryPerson/" + AssignedID + "", obj).then((response) => {
                if (response.data === true) {
                    Swal.fire({
                        title: "Driver Updated Sucessfully!",
                        icon: "success",
                        confirmButtonText: "OK",
                    });
                    document.getElementById("divLoading").className = "hide";
    ...
    return (
        <div id="city">
                        ...
                        <CCardBody>
                            <CDataTable
                                ...
                                scopedSlots={{
                                    ...
                                    "Status": (i) => {
                                        if (i.ORDER_STATUS == "4" || i.ORDER_STATUS == 4) {
                                            if (i.ASSIGNED_DOOR_DELIVERY_STATUS === "" || i.ASSIGNED_DOOR_DELIVERY_STATUS === null) {
                                                return (
                                                    <td>
                                                        <span className="pending">Not Assigned</span>
                                                    </td>
                                                );
                                            }
                                            else {
                                                return (
                                                    <td>
                                                        <span className="success">Assigned</span>
                                                    </td>
                                                );
                                            }
                                        }
            ...
            <CModal show={block4} onClose={() => setBlock4(!block4)} color="dark" style={{ height: 380 }}>
                <CModalHeader closeButton>
                    <CModalTitle>Driver</CModalTitle>
                </CModalHeader>
                <CModalBody>
                    <div>
                        <CFormGroup row>
                            ...
                            <CCol md="8">
                                <span style={{ fontSize: "14px", color: "#757575" }}>{OrderID}</span>
                            </CCol>
                        </CFormGroup>
                        <CDropdownDivider />
                        <CFormGroup row>
                            ...
                            <CCol md="8">
                                <span style={{ fontSize: "14px", color: "#757575" }}>{OrderDate.split("T")[0]}</span>
                            </CCol>
                        </CFormGroup>
                        <CDropdownDivider />
                        <CFormGroup row>
                            ...
                            <CCol md="8">
                                <span style={{ fontSize: "14px", color: "#757575" }}>{CustomerName}</span>
                            </CCol>
                        </CFormGroup>
                    </div>
                    <CForm
            ...
            <CModal show={block5} onClose={() => setBlock5(!block5)} color="dark" style={{ height: 380 }}>
                ...
                <CModalFooter>
                    {DriverForEdit.ORDER_STATUS == 4 || DriverForEdit.ORDER_STATUS == "4" ?
                        <CRow>
                            <CCol md="6">
                                <CButton className="btn btn-success" style={{ fontSize: "13px" }} onClick={() => EditDriver(DriverForEdit)}>
                                    Edit
                                </CButton>
                            </CCol>
                        </CRow>
            ...
            <CModal show={block6} onClose={() => setBlock6(!block6)} color="dark" style={{ height: 380 }}>
                <CModalHeader closeButton>
                    <CModalTitle>Update Assigned Driver</CModalTitle>
                </CModalHeader>
                <CModalBody>
                    <div>
                        <CFormGroup row>
                ...
                <CModalFooter>
                    <CRow>
                        <CCol md="6">
                            <CButton className="btn btn-success" style={{ fontSize: "13px" }} onClick={() => UpdateAssignedDriver()}>
                                Update
                            </CButton>
                        </CCol>
                    </CRow>

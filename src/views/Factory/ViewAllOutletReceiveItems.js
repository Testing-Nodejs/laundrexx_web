...
const ViewAllOutletReceivedItems = () => {
    ...
    const getAllOrders = () => {
        ...
        axios.get(MyApiUrl + "FactoryViewInTakeOrders/" + FactoryID + "").then((response) => {
            const items = response.data.map((item, index) => {
                return {
                    ...item,
                    "Sl No.": index + 1,
                    "DC Number": item.FACTORY_DC_NUMBER,
                    "DC Order Date": SplitDate1(item.FACTORY_DC_DATE),
                    "Outlet": item.STORE_NAME,
                    "Factory Staff": item.STORE_STAFF_NAME,
                    "Total Orders": item.FACTORY_DC_ORDER_COUNT,
    ...
    const filterData = (outlet, month, year) => {
        ...
        console.log(object);
        document.getElementById("divLoading").className = "show";
        axios.post(MyApiUrl + "FactoryViewInTakeOrdersFilter", object).then((response) => {
    ...
    return (
        <div id="city">
            ...
            <h1 id="ccmaster" style={{ marginTop: "3%" }}>View DC from Outlet</h1>
            ...
            <CRow>
                <CCol col="12">
                    <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
                        <CCardHeader>
                            <CRow>
                                <CCol col="6">View All DC from Outlet</CCol>
                            </CRow>
                        </CCardHeader>
                        <CCardBody>
                            <CDataTable
                                    ...
                                    Action: (item) => (
                                        <td>
                                            <CButton
                                                ...
                                                style={{ fontSize: "12px" }}
                                                onClick={() => {
                                                    history.push("/ReceiveItemsOutlet", {
                                                        data: item,
                                                    });
                                                }}
                                            >
...
export default ViewAllOutletReceivedItems;

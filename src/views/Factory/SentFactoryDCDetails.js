...
const ViewAllOrdersDC = () => {
    ...
    const getAllOrders = () => {
        ...
        axios.get(MyApiUrl + "ReturnToFactoryView/" + FactoryID + "").then((response) => {
            console.log(response.data)
            const items = response.data.map((item, index) => {
                return {
                    ...
                    "Order Time": item.FACTORY_TO_FACTORY_DC_TIME,
                    "From Factory": item.FromFactory,
                    "To Factory": item.ToFactory,
                    "From Factory Code": item.FromFactoryCode,
                    "To Factory Code": item.ToFactoryCode,
    ...
    const filterData = (factory, month, year) => {
        ...
        console.log(object);
        document.getElementById("divLoading").className = "show";
        axios.post(MyApiUrl + "ReturnToFactoryViewFilter", object).then((response) => {
    ...
    const ViewItems = async (id) => {
        await axios.get(MyApiUrl + "ReturnedToFactoryItemList/" + id + "").then((response) => {
            const items = response.data.map((item, index) => {
                return {
                    ...item,
                    "Sl No.": index + 1,
                    ...
                    "Due Date": SplitDate(item.ORDER_DATE),
    ...
    const printDC = async (list) => {
        setPrintDCData([]);
        setPrintDCData([list]);
        await axios.get(MyApiUrl + "ReturnedToFactoryItemList/" + list.FACTORY_TO_FACTORY_DC_PKID + "").then((response) => {
            const items = response.data.map((item, index) => {
                return {
                    ...item,
                    "Sl No.": index + 1,
    ...
    const printModalInvoice = async (id) => {
        ...
        await axios.post(MyApiUrl + "ReturnToFactoryViewByDCNumber", obj).then(async (response) => {
            console.log(response.data)
            if (response.data.length > 0) {
                setblock1(false);
                setModalPrintDCData(response.data);
                await axios.get(MyApiUrl + "ReturnedToFactoryItemList/" + response.data[0].FACTORY_TO_FACTORY_DC_PKID + "").then((response) => {
                    const items = response.data.map((item, index) => {
                        return {
                            ...item,
                            "Sl No.": index + 1,
        ...
        <div id="city">
                        ...
                        <center>
                            <table width="525" className="bottom" cellspacing="0" cellpadding="2" style={{ borderLeft: "1px solid #000", borderTop: "1px solid #000", }}>
                                ...
                                <tr style={{
                                    ...
                                    <td>: &nbsp;&nbsp;&nbsp;{PrintDCData[0].FromFactory}</td>
                                    <td style={{ fontWeight: "300" }}>To Station</td>
                                    <td style={{ borderRight: "1px solid #000" }}>: &nbsp;&nbsp;&nbsp;{PrintDCData[0].ToFactory}</td>
                                </tr>
                                <tr style={{
                                    fontWeight: "600",
                                    fontSize: "9px"
                            ...
                            <table width="525" border="0" cellspacing="0" cellpadding="2" style={{ borderTop: "1px solid #000", fontSize: 10 }}>
                                ...
                                {OrdersData.length > 0 ?
                                    OrdersData.map((item, index) => {
                                        Quantity = Quantity + item.ORDER_ITEM_QUANTITY;
                                        return (
                                            <tr>
                                                <td style={{
                                                    borderBottom: "thin solid",
                        ...
                        <center>
                            <table width="525" className="bottom" cellspacing="0" cellpadding="2" style={{ borderLeft: "1px solid #000", borderTop: "1px solid #000", }}>
                                ...
                                <tr style={{
                                    ...
                                    <td style={{ fontWeight: "300" }}>From Station</td>
                                    <td>: &nbsp;&nbsp;&nbsp;{ModalPrintDCData[0].FromFactory}</td>
                                    <td style={{ fontWeight: "300" }}>To Station</td>
                                    <td style={{ borderRight: "1px solid #000" }}>: &nbsp;&nbsp;&nbsp;{ModalPrintDCData[0].ToFactory}</td>
                                </tr>
                                <tr style={{
                                    fontWeight: "600",
                                    fontSize: "9px"
                            ...
                            <table width="525" border="0" cellspacing="0" cellpadding="2" style={{ borderTop: "1px solid #000", fontSize: 10 }}>
                                ...
                                {OrdersData.length > 0 ?
                                    OrdersData.map((item, index) => {
                                        FootQuantity = FootQuantity + item.ORDER_ITEM_QUANTITY;
                                        return (
                                            <tr>
                                                <td style={{
                                                    borderBottom: "thin solid",
            ...
            <CRow>
                <CCol col="12">
                    <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
                        <CCardHeader>
                            <div style={{ float: "left" }}>View All Factory Returned DC</div>
                        ...
                        <CCardBody>
                            <CDataTable
                                ...
                                scopedSlots={{
                                    "Total Items": (i) => (
                                        <td>
                                            <CButton
                                                ...
                                                style={{ fontSize: "12px" }}
                                                onClick={() => {
                                                    ViewItems(i.FACTORY_TO_FACTORY_DC_PKID);
                                                }}
                                            >
                                    ...
                                    "Status": (i) => {
                                        if (i.FACTORY_TO_FACTORY_DC_STATUS === "0" || i.FACTORY_TO_FACTORY_DC_STATUS === 0) {
                                            return (
                                                <td>
                                                    <span className="pending">Sent to Factory</span>
                                                </td>
                                            );
                                        } else if (i.FACTORY_TO_FACTORY_DC_STATUS === "1" || i.FACTORY_TO_FACTORY_DC_STATUS === 1) {
                                            return (
                                                <td>
                                                    <span className="assigned">Received in Factory</span>
                                                </td>
                                            );
                                        }
                                    },
            ...
            <CModal show={block1} onClose={() => setblock1(!block1)} color="dark" style={{ height: 250 }}>
                ...
                <CModalBody>
                    <CFormGroup row>
                        <CCol xs="12" md="12">
                            <CLabel>Item DC Number <span style={{ color: "red" }}>*</span></CLabel>
                            <CInput
                                id="ModalOrderID"
                                placeholder="Enter Item No./ Scan OR Code"
                                value={ModalOrderID}
                                onChange={(event) => {

...
const DoorDelivery = () => {
    ...
    const getAllOrders = () => {
        ...
        axios.get(MyApiUrl + "GetAllDoorDeliveryOrders/"+ StoreID +"").then((response) => {
    ...
    const filterData = (customer, month, year) => {
        ...
        console.log(object);
        document.getElementById("divLoading").className = "show";
        axios.post(MyApiUrl + "GetAllDoorDeliveryOrdersWithFilter", object).then((response) => {
            if (response.data.length > 0) {
                const items = response.data.map((item, index) => {
                    return {
                        ...item,
    ...
    const UpdateHandover = (Orderid) => {
        alert("kkk")
            axios.get(MyApiUrl + "updateOutletHandoverToDriver/"+ Orderid +"").then((response) => {
                                ...
                                scopedSlots={{
                                    ...
                                    "Status": (i) => {
                                        if (i.ASSIGNED_DOOR_DELIVERY_STATUS === "" || i.ASSIGNED_DOOR_DELIVERY_STATUS === null) {
                                            ...
                                            return (
                                                <td>
                                                    <span className="status_two">Assigned</span>
                                                </td>
                                            )
                                        }
                                    },

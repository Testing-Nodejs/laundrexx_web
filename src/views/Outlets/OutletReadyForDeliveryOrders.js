...
const OutletReadyForDelivery = () => {
    ...
    const getAllOrders = () => {
        ...
        axios.get(MyApiUrl + "OutletGetReadyForDeliveryOrders/" + StoreID + "").then((response) => {
            if (response.data.length > 0) {
                const items = response.data.map((item, index) => {
                    return {
                        ...item,
                        "Sl No.": index + 1,
                        "Order Date": SplitDate(item.ORDER_DATE),
    ...
    const filterData = (customer, month, year) => {
        ...
        document.getElementById("divLoading").className = "show";
        axios.post(MyApiUrl + "OutletGetReadyForDeliveryOrdersFilter", object).then((response) => {
            if (response.data.length > 0) {
                const items = response.data.map((item, index) => {
                    return {
                        ...item,
                        ...
                        "Round Invoice Value": item.ORDER_ROUND_OFF_INVOICE,
                        "Grand Total Amount": item.ORDER_GRAND_TOTAL_AMOUNT,
                        "Status": item.ORDER_STATUS === "0" || item.ORDER_STATUS === 0 ? "Pending" : "-",
                    };
                });
                setResponseData(items);
                setExcelData(items);
    ...
    const ToExcel = () => {
        ...
        csvData.push(
            '"Sl No","Order Number","Order Date","Due Date","Order Invoice Number","Order Quantity",,"Order Count","Outlet","Outlet Phone Number","Customer Name","Contact Number","Service Category","Service Type","Pickup Request","Pickup ID","Pickup Address","Pickup Date","Driver Name","Coupon","Coupon Discount","Bulk Item","Door Delivery","Order Amount","Surcharge","Order Discount","Total Order Amount","CGST","SGST","Total Invoice Value","Round Invoice Value","Grand Total Amount"'
        );
        ExcelData.map((item) => {
        ...
        const fileName = "OutletOrderReadyForDelivery.csv";
    ...
    return (
        <div id="city">
            ...
            <h1 id="ccmaster" style={{ marginTop: "3%" }}>Ready For Delivery Orders</h1>
                        ...
                        <table id="grdpaymentDetails" border="1" cellspacing="2px" cellpadding="2px" style={{ borderRight: "1px solid #999999", fontSize: 12, borderCollapse: "collapse" }} class="txtcss totalTable">
                            <thead>
                                ...
                                <tr style={{ color: "#000", fontWeight: "bold" }}>
                                    ...
                                    <th scope="col" style={{ width: "4%" }}>Order Count</th>
            ...
            <CRow>
                <CCol col="12">
                    <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
                        <CCardHeader>
                        <CCol md="12" lg="8" style={{ float: 'left' }}>View Ready For Delivery Orders</CCol>
                                <CButton
                                    onClick={ToExcel}
                                    color="info"
                                    style={{
...
export default OutletReadyForDelivery;

...
const TagsReprinting = (props) => {
            ...
            .then((response) => {
                const items = response.data.map((item, index) => {
                    return {
                        ...
                        OutletCode: item.ORDER_ITEM_NUMBER.split("-")[0],
                    };
                });
    ...
    const TagPrint = async (obj) => {
        await setPlaceOrderResponse([obj]);
        console.log([obj]);
        const screenWidth = screen.width;
        const tagsWindowLeft = Math.floor((screenWidth - 780) / 4);
        var tagsWindow = window.open("", "Tags Window", "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=auto,top=" + (screen.height - 400) + ",left=" + tagsWindowLeft);
        ...
        tagsWindow.document.write('<html><head><title>Tags Print</title>');
        tagsWindow.document.write(styles);
        tagsWindow.document.write('</head><body >');
        tagsWindow.document.write(document.getElementById("print1").innerHTML);
    ...
    const AllTagsPrint = async () => {
        await setPlaceOrderResponse(ResponseData);
        const screenWidth = screen.width;
        const tagsWindowLeft = Math.floor((screenWidth - 780) / 4);
        var AllTagsWindow = window.open("", "All Tags Window", "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=auto,top=" + (screen.height - 400) + ",left=" + tagsWindowLeft);
        ...
        AllTagsWindow.document.write('<html><head><title>All Tags Window</title>');
        AllTagsWindow.document.write(styles);
        AllTagsWindow.document.write('</head><body >');
        AllTagsWindow.document.write(document.getElementById("print1").innerHTML);

        AllTagsWindow.document.write('</body></html>');
        AllTagsWindow.document.close();
    ...
    return (
        <div>
                                                    ...
                                                    <CButton
                                                        ...
                                                        onClick={() => {
                                                            var obj = {
                                                                ...
                                                                ORDER_ITEM_DEFECT: i.ORDER_ITEM_DEFECT,
                                                                ORDER_ITEM_NUMBER: i.ORDER_ITEM_NUMBER,
                                                                ORDER_ITEM_QR: i.ORDER_ITEM_QR,
                                                                ORDER_ITEM_COUNT: i.ORDER_ITEM_COUNT,
                                                                OutletCode: i.ORDER_ITEM_NUMBER.split("-")[0],
                                                            }
                                                            TagPrint(obj);
                                                        }}
                                                    >
            ...
            <div style={{ width: "190px", position: "absolute", display: "none", marginTop: "10%", textAlign: "center", marginBottom: "5%", height: "100%", width: "100%" }} id="print1">
                {PlaceOrderResponse.length > 0 ?
                    PlaceOrderResponse.map((item) => {
                        let jsxElements = [];
                        for (let i = 0; i < parseInt(item.ORDER_ITEM_COUNT); i++) {
                            jsxElements.push(
                                <React.Fragment key={i}>
                                    <div style={{ width: "245px", textAlign: "center" }}>
                                        <img width="170px" alt="dsa" src="./assets/images/laundrexx_scissor.jpg" />
                                        <center>
                                            ...
                                            <table style={{ width: "10%" }}>
                                                <tr>
                                                    <td align="center">
                                                        <span style={{ fontFamily: "verdana", fontSize: "16px", fontWeight: 900 }}>{CUSTOMER_NAME}</span><br />
                                                        ...
                                                        <table style={{ width: "10%", marginTop: "15%" }}>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <span style={{ fontFamily: "verdana", fontSize: "14px" }}>{SplitDate(ORDER_DATE)}</span><br />
                                                                </td>
                                                                <td></td>
                                                                <td></td>
                                                                <td></td>

...
const OutletViewIntakeProcess = () => {
    ...
    const getAllOrders = () => {
        ...
        axios.get(MyApiUrl + "ViewOutletDCForIntakeWithoutFactory/" + StoreID + "").then((response) => {
    ...
    const filterData = (month, year) => {
        ...
        document.getElementById("divLoading").className = "show";
        axios.post(MyApiUrl + "ViewOutletDCForIntakeWithoutFactoryFilter", object).then((response) => {
    ...
    return (
        <div id="city">
            ...
            <h1 id="ccmaster" style={{ marginTop: "3%" }}>Intake Process</h1>
                        ...
                        <CCardBody>
                            <CDataTable
                                ...
                                scopedSlots={{
                                    "Action": (i) => (
                                        <td>
                                            {i.FACTORY_DC_STATUS == "2" || i.FACTORY_DC_STATUS == 2 ?
                                                <CButton
                                                ...
                                                <CButton
                                                    ...
                                                    style={{ fontSize: "12px" }}
                                                    onClick={() =>
                                                        history.push("/OutletIntakeProcessWithoutDC", {
                                                            data: i,
                                                        })
                                                    }
                                                >
                                    ...
                                    "Status": (i) => {
                                        if (i.FACTORY_DC_STATUS === "0" || i.FACTORY_DC_STATUS === 0) {
                                            ...
                                            return (
                                                <td>
                                                    <span className="pending">Received In Factory</span>
                                                </td>
                                            );
                                        }
                                        else if (i.FACTORY_DC_STATUS === "2" || i.FACTORY_DC_STATUS === 2) {
                                            return (
                                                <td>
                                                    <span className="pending">DC Received</span>
                                                </td>
                                            );
                                        }
                                    },
                                    "Total Orders": (i) => (
                                        <td>
                                            {i.FACTORY_DC_ORDER_COUNT === "" || i.FACTORY_DC_ORDER_COUNT === null ?
                                                "-" : <CButton
                                                    className="OrderBtn"
                                                    style={{ fontSize: "12px" }}
                                                    onClick={() => {
                                                        ViewOrderCount(i.DCInnerItems);
                                                    }}
                                                >
                            ...
                            <CCol md="12">
                                <CDataTable
                                    ...
                                    scopedSlots={{
                                        ...
                                        "Order Date": (i) => (
                                            <td>{SplitDate(i.ORDER_DATE)}</td>
                                        ),
                                        "Order Number": (i) => (
                                            <td>{i.ORDER_ORDER_NUMBER}</td>
                                        ),
                                        "Invoice Number": (i) => (
                                            <td>{i.ORDER_INVOICE_NUMBER}</td>
                                        ),
                                        "Due Date": (i) => (
                                            <td>{SplitDate(i.ORDER_DUE_DATE)}</td>
                                        ),
                                        "Grand Total Amount": (i) => (
                                            <td>{i.ORDER_GRAND_TOTAL_AMOUNT}</td>
                                        ),
                                        "Total Quantity": (i) => (
                                            <td>{i.ORDER_ITEMS}</td>
                                        ),

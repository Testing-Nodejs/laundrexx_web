...
const OutletDeliveryChallan = () => {
        ...
        axios.get(MyApiUrl + "GetOutletToFactoryDC/" + StoreID + "").then((response) => {
            const items = response.data.map((item, index) => {
                return {
                    ...
                    "Order Quantity": item.TotalCount,
    ...
    const ToExcel = () => {
        ...
        csvData.push(
            '"Sl No","DC Number","DC Date","Staff Name","Factory Name","Factory Code","Total Quantity","Total Count","Total Bags"'
        );
        ExcelData.map((item) => { 
            return (
                cnt++,
                csvData.push(
                    ...
                    cnt +
                    '","' +
                    item.FACTORY_DC_NUMBER +
                    '","' +
                    SplitDate1(item.FACTORY_DC_DATE) +
                    '","' +
                    item.STORE_STAFF_NAME +
                    '","' +
                    item.FACTORY_NAME +
                    '","' +
                    item.FACTORY_CODE +
    ...
    return (
        <div id="city">
            ...
            <h1 id="ccmaster" style={{ marginTop: "3%" }}>Delivery Challan</h1>
            ...
            <CRow>
                <CCol col="12">
                    <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
                        <CCardHeader>
                           <CCol md="12" lg="8" style={{ float: 'left' }}>View All Delivery Challan's</CCol>
                                ...
                                scopedSlots={{
                                    ...
                                    "Status": (i) => {
                                        if (i.FACTORY_DC_STATUS === "0" || i.FACTORY_DC_STATUS === 0) {
                                            ...
                                            return (
                                                <td>
                                                    <span className="dc_one">Recieved in Factory</span>
                                                </td>
                                            );
                                        }
                                    },

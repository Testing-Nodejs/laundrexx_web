...
const OutletOrderReports = () => {
    ...
    const getAllOrders = () => {
        ...
        axios.get(MyApiUrl + "OutletOrdersCurrentDay/" + StoreID + "").then((response) => {
    ...
    const filterData = (customer, month, year) => {
        ...
        document.getElementById("divLoading").className = "show";
        axios.post(MyApiUrl + "OutletOrderFilter", object).then((response) => {
            if (response.data.length > 0) {
                const items = response.data.map((item, index) => {
                    return {
                        ...item,
    ...
    const ToExcel = () => {
        ...
        csvData.push(
            '"Sl No","Status","Order Number","Order Date","Due Date","Order Invoice Number","Order Quantity","Order Count","Outlet","Outlet Phone Number","Customer Name","Contact Number","Service Category","Service Type","Pickup Request","Pickup ID","Pickup Address","Pickup Date","Driver Name","Coupon","Coupon Discount","Bulk Item","Door Delivery","Order Amount","Surcharge","Order Discount","Total Order Amount","CGST","SGST","Total Invoice Value","Round Invoice Value","Grand Total Amount"'
        );
        ExcelData.map((item) => {
            let Status = "";
        ...
        const fileName = "OutletOrderReport.csv";
    ...
    return (
        <div id="city">
            ...
            <h1 id="ccmaster" style={{ marginTop: "3%" }}>Billing Orders</h1>
                        ...
                        <table id="grdpaymentDetails" border="1" cellspacing="2px" cellpadding="2px" style={{ borderRight: "1px solid #999999", fontSize: 12, borderCollapse: "collapse" }} class="txtcss totalTable">
                            <thead>
                                ...
                                <tr style={{ color: "#000", fontWeight: "bold" }}>
                                    ...
                                    <th scope="col" style={{ width: "4%" }}>Total Count</th>
            ...
            <CRow>
                <CCol col="12">
                    <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
                        <CCardHeader>
                            <CCol md="12" lg="8" style={{ float: 'left' }}>View All Billing Orders</CCol>
                            <CButton
...
export default OutletOrderReports;

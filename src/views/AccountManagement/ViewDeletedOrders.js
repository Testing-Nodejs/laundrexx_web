...
const ViewDeletedOrders = () => {
    ...
    const getDeletedOrders = () => {
        ...
        if (UserType == "Manager") {
            axios.get(MyApiUrl + "GetDeletedOrdersForManager/" + UserID + "").then((response) => {
            ...
            axios.get(MyApiUrl + "GetDeletedOrders").then((response) => {
                const items = response.data.map((item, index) => {
                    return {
                        ...item,
                        "Sl No.": index + 1,
    ...
    const filterData = (outlet, month, year) => {
        ...
        if (UserType == "Manager") {
            ...
            axios.post(MyApiUrl + "AllDeletedOrdersFilterForManager", object).then((response) => {
                if (response.data.length > 0) {
                    const items = response.data.map((item, index) => {
                        return {
                            ...item,
            ...
            axios.post(MyApiUrl + "AllDeletedOrdersFilter", object).then((response) => {
                if (response.data.length > 0) {
                    const items = response.data.map((item, index) => {
                        return {
                            ...item,
    ...
    const ToExcel = () => {
        ...
        const fileName = "DeletedOrdersReport.csv";
    ...
    return (
        <div id="city">
            ...
            <h1 id="ccmaster">Deleted Orders</h1>
            ...
            <CRow>
                <CCol col="12">
                    <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
                        <CCardHeader>
                            <CCol md="12" lg="8" style={{ float: 'left' }}>View All Deleted Orders
                            </CCol>
                            <CButton
                                        ...
                                        <td>
                                            {i.ORDER_COUPON_FKID === "" || i.ORDER_COUPON_FKID === null ?
                                                "-" : <CButton
                                                    ...
                                                    style={{ fontSize: "12px" }}
                                                    onClick={() => {
                                                        ViewCoupon(i.COUPON_PKID, i.COUPON_TYPE_NAME, i.COUPONS_NAME, i.COUPONS_CODE, i.COUPONS_PRICE_OR_PERCENTAGE, i.COUPONS_DISCOUNT, i.COUPONS_VALIDITY, i.COUPONS_VALIDITY_DATE, i.COUPONS_ACTIVE);
                                                    }}
                                                >
                                                    {i.COUPONS_NAME}
                                                </CButton>
...
export default ViewDeletedOrders;
